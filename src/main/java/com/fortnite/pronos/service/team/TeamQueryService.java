...
@SuppressWarnings({"java:S2201", "java:S4144"})
public class TeamQueryService implements TeamQueryUseCase {
  ...
  public List<TeamDto> getAllTeams(int season) {
    ...
    List<Team> teams = teamDomainRepository.findBySeasonWithFetch(season);
  ...
  public List<TeamDto> getTeamsBySeason(int season) {
    return teamDomainRepository.findBySeasonWithFetch(season).stream()
        .map(this::toTeamDto)
        .toList();
  }
  ...
  public List<TeamDto> getTeamsByGame(UUID gameId) {
    ...
    List<Team> teams = teamDomainRepository.findByGameIdWithFetch(gameId);
  ...
  public List<com.fortnite.pronos.model.Team> findTeamsBySeasonWithFetch(int season) {
    return teamRepository.findBySeasonWithFetch(season);
  }

  /** Returns Team entity by ID (legacy API) */
  public Optional<com.fortnite.pronos.model.Team> findTeamByIdWithFetch(UUID teamId) {
    return teamRepository.findByIdWithFetch(teamId);
  }

  /** Returns Team entity by owner and season (legacy API) */
  public Optional<com.fortnite.pronos.model.Team> findTeamByOwnerAndSeason(
      com.fortnite.pronos.model.User owner, int season) {
  ...
  private com.fortnite.pronos.model.User findUserByUsername(String username) {
    return userRepository
  ...
  private boolean isParticipantTeam(Team team) {
    return userRepository
        .findById(team.getOwnerId())
        .map(owner -> owner.getRole() == com.fortnite.pronos.model.User.UserRole.USER)
        .orElse(false);
  }
}
