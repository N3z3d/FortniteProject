...
6. **participantUsername dans le payload de tour** : Le backend diffuse `participantUsername` dans le payload snake broadcast après `validateAndAdvance()` — le frontend résout le prochain tour sans gap UUID -> username.
...
### Task 3 — Align contract : `participantUsername` dans le payload snake [BACKEND · 45 min]
...
- **Fix 3** (`participantUsername`) : élimine le gap de résolution côté frontend — le prochain tour est connu sans lookup UUID -> username
...
### Review Findings (initial triage)

- [ ] [Review][Decision] AC #0/#1 waived instead of satisfied â€” la story documente encore que le protocole diagnostic obligatoire "logs avant tout fix" n'a pas ete rejoue avant implementation, donc les AC 0/1 ne peuvent pas etre considerees comme satisfaites sans waiver explicite de review.
- [ ] [Review][Decision] AC #6 still mismatched with shipped contract â€” l'acceptance criterion exige toujours `nextPlayerId` dans l'event STOMP alors que l'implementation shippee repose sur `participantUsername` et indique explicitement qu'aucun nouveau champ backend distinct n'a ete ajoute.
- [ ] [Review][Patch] Region switch leaves the page in a stale state after a failed reload [frontend/src/app/features/draft/components/snake-draft-page/snake-draft-page.component.ts:117]
- [ ] [Review][Patch] Null PR ranks collapse to `0` and can block otherwise eligible players [frontend/src/app/features/draft/services/draft.service.ts:488]

### Review Resolution (2026-04-05)

- [x] [Review][Decision] AC #0/#1 waiver accepted - the existing process waiver remains explicit because replaying a fake "before fix" diagnostic now would be misleading.
- [x] [Review][Patch] AC #6 aligned with shipped `participantUsername` contract [_bmad-output/implementation-artifacts/sprint14-fix-draft-crash-tour2.md:31]
- [x] [Review][Patch] Region switch now restores the previous UI state when a regional reload fails [frontend/src/app/features/draft/components/snake-draft-page/snake-draft-page.component.ts:126]
- [x] [Review][Patch] Players without PR snapshots stay eligible client-side [frontend/src/app/features/draft/services/draft.service.ts:495]
